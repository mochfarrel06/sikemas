...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Tambah Antrean Pasien Admin</h1>
                </div>
                <div class="col-sm-6">
        ...
        <div class="container-fluid">
            <div class="row">
                <section class="col-lg-12">
                    <div class="card">
                        <form id="main-form" method="POST" action="{{ route('data-patient.storeAntreanAdmin') }}">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="custom-select @error('patient_id') is-invalid @enderror"
                                                name="patient_id" id="patient_id">
                                                <option value="">-- Pilih Pasien --</option>
                                                @foreach ($patients as $patient)
                                                    <option value="{{ $patient->id }}">{{ $patient->nama_depan }}
                                                        {{ $patient->nama_belakang }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="card-footer">
                                <button type="submit" class="btn btn-primary2 mr-2">Simpan</button>
    ...
    <script>
        $(document).ready(function() {
            $('#doctor_id, #date').on('change', function() {
                ...
                if (doctorId && date) {
                    $.ajax({
                        url: "{{ route('data-patient.createAntreanAdmin') }}",
                        method: 'GET',
                        data: {
                            doctor_id: doctorId,
                            date: date
